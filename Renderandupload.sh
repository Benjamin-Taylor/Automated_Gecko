sudo kill $(pgrep -f 'imageAnnotated3.py')
...
/usr/local/bin/youtube-upload --title=$DATE --description="Night timelapse of Crested Gecko in custom varvarium. Raspberry pi 2, NoIR Camera, 4 Channel Relay+36 IR LED Illuminator board. timelapse=4 frames per minute, TempMat=DHT11(temperature only) of Heat mat, Ambient=DHT(temperature only) at top of varvarium."  --client-secrets=/home/pi/Gecko_main/client_secrets.json --credentials-file=/home/pi/.youtube-upload-credentials.json --playlist "Gek_vids" /home/pi/Gecko_main/video_bin/$DATE.mp4 &> ~/Gecko_main/logs/uploadlogs/uploadlog_$DATE.txt
